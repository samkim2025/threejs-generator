...
def load_from_history(index):
    if 0 <= index < len(st.session_state.scene_history):
        ...
        return True
    return False# Enhanced prompt function using explicit examples
...
Your response should start with <!DOCTYPE html> and end with </html>."""}
    ...
    async with httpx.AsyncClient(timeout=180.0) as client:
        ...
        if "content" in response_data and len(response_data["content"]) > 0:
            ...
            return None, debug_info# Example prompts for the scene generator
EXAMPLE_MAPPINGS = [
    {
